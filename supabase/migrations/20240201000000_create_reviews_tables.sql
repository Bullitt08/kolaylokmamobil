...
CREATE POLICY "Kullanıcılar kendi yorumlarını ekleyebilir" ON reviews
...
CREATE POLICY "Sadece adminler rapor görebilir" ON review_reports
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users
            WHERE users.id = auth.uid()
...
CREATE POLICY "Adminler rapor durumunu güncelleyebilir" ON review_reports
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM public.users
            WHERE users.id = auth.uid()
...
CREATE POLICY "Sadece adminler bildirimlere erişebilir" ON admin_notifications
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.users
            WHERE users.id = auth.uid()
